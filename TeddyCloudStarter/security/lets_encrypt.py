...
class LetsEncryptManager:
    ...
    def create_letsencrypt_certificate_webroot(self, domain, email=None, sans=None, staging=False, force_renewal=False, project_path=None, docker_manager=None):
        """
        ...
        if services.get('nginx-edge', {}).get('state', '').lower() != 'running':
            console.print(f"[bold yellow]{self._translate('Starting nginx-edge service for webroot challenge...')}[/]")
            started = docker_manager.start_service('nginx-edge', project_path)
            if not started:
                console.print(f"[bold red]{self._translate('Failed to start nginx-edge service. Cannot proceed with certificate request.')}[/]")
                return False
        ...
        cmd = [
            ...
            certbot_image,
            "certonly", "--webroot", "-w", webroot_path,
            "-d", domain
        ]
        if email:
            cmd += ["--email", email]
        ...
        console.print(f"[bold cyan]{self._translate('Requesting Let\'s Encrypt certificate for')} {domain}...[/]")
        try:
            result = subprocess.run(cmd, check=False, capture_output=True, text=True)
            if result.returncode == 0:
                console.print(f"[bold green]{self._translate('Certificate successfully obtained for')} {domain}[/]")
                return True
            else:
                console.print(f"[bold red]{self._translate('Failed to obtain certificate for')} {domain}[/]")
                console.print(result.stdout)
                console.print(result.stderr)
                return False
        except Exception as e:
            console.print(f"[bold red]{self._translate('Error running certbot:')} {e}[/]")
            return False
