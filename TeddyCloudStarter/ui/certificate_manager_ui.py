...
def show_certificate_management_menu(config, translator, security_managers):
    ...
    if selected_id == 'create_client_cert':
        ...
        action_id = show_letsencrypt_management_menu(config, translator, security_managers["lets_encrypt_manager"])
        
        if action_id == 'prod_webroot':
            request_letsencrypt_certificate(config, translator, security_managers["lets_encrypt_manager"], staging=False, mode="webroot")
        elif action_id == 'prod_standalone':
            request_letsencrypt_certificate(config, translator, security_managers["lets_encrypt_manager"], staging=False, mode="standalone")
        elif action_id == 'staging_webroot':
            request_letsencrypt_certificate(config, translator, security_managers["lets_encrypt_manager"], staging=True, mode="webroot")
        elif action_id == 'staging_standalone':
            request_letsencrypt_certificate(config, translator, security_managers["lets_encrypt_manager"], staging=True, mode="standalone")
        elif action_id == 'refresh':
            refresh_letsencrypt_certificates(config, translator, security_managers["lets_encrypt_manager"])
...
def show_letsencrypt_management_menu(config, translator, lets_encrypt_manager):
    ...
    """
    choices = [
        {'id': 'prod_webroot', 'text': translator.get("Request production certificate (webroot mode)")},
        {'id': 'prod_standalone', 'text': translator.get("Request production certificate (standalone mode)")},
        {'id': 'staging_webroot', 'text': translator.get("Request staging certificate (webroot mode)")},
        {'id': 'staging_standalone', 'text': translator.get("Request staging certificate (standalone mode)")},
        {'id': 'refresh', 'text': translator.get("Force refresh Let's Encrypt certificates")},
...
def test_domain_for_letsencrypt(config, translator, lets_encrypt_manager):
    ...
    if result:
        console.print(f"[bold green]{translator.get('Success')}: {translator.get('Domain')} {domain} {translator.get('is properly set up for Let\'s Encrypt')}[/]")
    else:
        console.print(f"[bold red]{translator.get('Warning')}: {translator.get('Domain')} {domain} {translator.get('may not be properly set up for Let\'s Encrypt')}[/]")
        console.print(f"[yellow]{translator.get('Please ensure the domain resolves to this server and ports 80 and 443 are accessible from the internet.')}[/]")