...
class TeddyLogger:
    ...
    def __init__(
        ...
        if config_manager and hasattr(config_manager, "config"):
            if "app_settings" in config_manager.config:
                self.log_level = config_manager.config["app_settings"].get("log_level", "info").lower()
    ...
    def _handle_message(self, message):
        ...
        if isinstance(message, Panel):
            ...
            return str(message.renderable) if hasattr(message, 'renderable') else str(message)
        return message
    ...
    def print_warning(self, message, title=None, label=None):
        ...
        if isinstance(message, Panel):
            ...
            if self.use_panels:
                panel = self._create_panel(translated_message, "warning", title or "WARNING")
                console.print(panel)
            elif self.use_inline:
    ...
    def print_error(self, message, title=None, label=None):
        ...
        if isinstance(message, Panel):
            ...
            if self.use_panels:
                panel = self._create_panel(translated_message, "error", title or "ERROR")
                console.print(panel)
            elif self.use_inline:
    ...
    def print_success(self, message, title=None, label=None):
        ...
        if isinstance(message, Panel):
            ...
            if self.use_panels:
                panel = self._create_panel(translated_message, "success", title or "SUCCESS")
                console.print(panel)
            elif self.use_inline:
    ...
    def print_debug(self, message, title=None, label=None):
        ...
        if isinstance(message, Panel):
            ...
            if self.use_panels:
                panel = self._create_panel(translated_message, "debug", title or "DEBUG")
                console.print(panel)
            elif self.use_inline:
    ...
    def print_critical(self, message, title=None, label=None):
        ...
        if isinstance(message, Panel):
            ...
            if self.use_panels:
                panel = self._create_panel(translated_message, "critical", title or "CRITICAL")
                console.print(panel)
            elif self.use_inline:
        ...
        if level.lower() in LOG_LEVELS:
            ...
            if self.config_manager and hasattr(self.config_manager, "config"):
                if "app_settings" in self.config_manager.config:
                    self.config_manager.config["app_settings"]["log_level"] = level.lower()
...
def get_logger(name: str = None, config_manager=None, log_to_file: bool = False, log_path: Optional[str] = None, use_panels: bool = False, use_inline: bool = True) -> TeddyLogger:
    """
    ...
    """
    return TeddyLogger(name or "TeddyCloudStarter", config_manager, log_to_file, log_path, use_panels, use_inline)