...
def handle_letsencrypt_setup(nginx_config, translator, lets_encrypt_manager):
    ...
    if not domain:
        console.print(f"[bold red]{translator.get('Error: No domain specified for Let\'s Encrypt')}[/]")
        return False
    
    # Check if domain is publicly resolvable
    if not check_domain_suitable_for_letsencrypt(domain, translator, nginx_config["https_mode"]):
        return False
    ...
    if not confirm_letsencrypt_requirements(translator):
        console.print(f"[bold yellow]{translator.get('Let\'s Encrypt requires these prerequisites to function properly.')}[/]")
        return False
    
    # Ask if the user wants to test if Let's Encrypt can issue a certificate
    if confirm_test_certificate(translator):
        console.print(f"[bold cyan]{translator.get('Testing Let\'s Encrypt certificate request in staging environment...')}[/]")
        ...
        if lets_encrypt_manager:
            ...
            if success:
                console.print(f"[bold green]{translator.get('Let\'s Encrypt test successful! Your domain is properly configured.')}[/]")
                return True
            else:
                console.print(f"[bold red]{translator.get('Let\'s Encrypt test failed. Your domain may not be properly configured.')}[/]")
                console.print(f"[yellow]{translator.get('Switching to self-signed certificates as a fallback.')}[/]")
                return False
                
        else:
            console.print(f"[bold yellow]{translator.get('Let\'s Encrypt manager not available. Skipping test.')}[/]")
...
def switch_to_letsencrypt_https_mode(config, translator, lets_encrypt_manager):
    ...
    if result:
        ...
        console.print(f"[bold green]{translator.get('HTTPS mode successfully switched to Let\'s Encrypt')}[/]")
