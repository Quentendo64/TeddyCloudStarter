...
def check_https_prerequisites(https_mode):
    ...
    if https_mode in ("self_signed", "user_provided"):
        try:
            ...
            return False, "OpenSSL is not available. Cannot generate or validate certificates."
    return True, ""
...
def configure_nginx_mode(config, translator, security_managers):
    ...
    for warning in warnings:
        console.print(f"[bold yellow]{translator.get('Warning')}: {translator.get(warning)}[/]")
    ...
    if nginx_config.get("https_mode") in ["letsencrypt", "self_signed", "user_provided"]:
            ...
            if nginx_config["https_mode"] == "self_signed":
                ...
                if not success:
                    if result_mode == "retry":
                        ...
                        nginx_config["https_mode"] = result_mode  # Switch to user_provided
                        continue
