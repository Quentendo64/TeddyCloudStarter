...
def extract_strings_basic():
    ...
    with open(POT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write(
            "# This file is distributed under the same license as the TeddyCloudStarter package.\n"
        )
        f.write('msgid ""\n')
        f.write('msgstr ""\n')
...
def update_po_files():
    ...
    for lang_dir in glob.glob(str(LOCALES_DIR / "*")):
        ...
        if os.path.exists(po_file):
            ...
            try:
                subprocess.run(
                    ["msgmerge", "--update", "--backup=none", po_file, str(POT_FILE)],
                    check=True,
                )
...
def main():
    ...
    print("\nDone!")
    print(
        "To add a new language, run: python extract_translations.py create_lang <lang_code>"
    )
